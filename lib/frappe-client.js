...
export async function registerCustomer(payload) {
  ...
  const response = await fetch(new URL('/api/method/seagera_portal.api.register_customer', FRAPPE_BASE_URL).toString(), {
    method: 'POST',
    headers: {
...
export async function fetchUserRoles(sid, email) {
  if (!sid || !email) {
    return []
  }
  ...
  try {
    ...
    const payload = await response.json().catch(() => null)
    const roles = Array.isArray(payload?.message) ? payload.message : []
    return roles
  } catch (error) {
