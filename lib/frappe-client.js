...
export async function requestService(payload, providedSid) {
  const sid = providedSid ?? (await readCookieFromHeaders())
  if (!sid) {
    throw new Error('Missing session cookie')
  }
  ...
  if (!response.ok) {
    const message = data?.message ?? data?.error ?? 'Unable to submit service request.'
    throw new Error(typeof message === 'string' ? message : 'Unable to submit service request.')
  }
